...
def main(opt):
    ...
    out_path  = opt.out
    prefix    = opt.prefix
    out_peak    = out_path + '/' + prefix + '.macs2.peaks.bed'
    out_peak_e5 = out_path + '/' + prefix + '.macs2.peaks_enrich5.bed'
    ...
    state_txt   = out_path + '/' + prefix + '.macs2.peaks.state.txt'
    state_txt_e5   = out_path + '/' + prefix + '.macs2.peaks.enrich5.state.txt'

    anno_txt    = out_path + '/' + prefix + '.macs2.peaks.anno.txt'
    anno_enrich_txt  = out_path + '/' + prefix + '.macs2.peaks.enrich.element.txt'
    ...
    with open(inpeak,'r') as inf:
        for lines in inf:
            if not (lines.startswith("#") or "start" in lines):
                ...
                line=lines.split('\t')
                if line[0] in chrs:
                    s=line[0]+'\t'+line[1]+'\t'+line[2]+'\t'+line[9]+'\t'+line[7]+'\t'+strand+'\n'
                    out_peak_f.write(s)
                    if float(line[7]) >=5:
